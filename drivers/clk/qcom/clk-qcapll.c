...
clk_qcapll_set_rate(struct clk_hw *hw, unsigned long rate, unsigned long p_rate)
{
	...
	regmap_update_bits(pll->clkr.regmap, pll->mod_reg, PLL_MODULATION_START, 0);

	/* Wait until PLL is locked. Refer DPLL document for IPQ4019. This is recommended settling time for the PLL*/
	udelay(50);
